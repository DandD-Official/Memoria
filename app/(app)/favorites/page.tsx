...
export default async function FavoritesPage() {
  ...
  return (
    <PageShell className="max-w-5xl">
      <PageHeader>
        <PageHeaderContent><p className="eyebrow">Keep a few things close</p><PageTitle className="mt-3">Your favorites</PageTitle><PageDescription>The notes and study resources you want close at hand.</PageDescription></PageHeaderContent>
      </PageHeader>

      {total === 0 ? (
        <EmptyState icon={Star} title="No favorites yet" description="Favorite a book, note, reviewer, or quiz and it will appear here." actionLabel="Browse books" actionHref="/books" />
      ) : (
        <div className="space-y-8">
          {books.length > 0 && <section><SectionHeader title="Books" description={`${books.length} favorite ${books.length === 1 ? "Book" : "Books"}`} /><div className="resource-list">{books.map((book) => <ResourceCard key={book.id} href={`/books/${book.id}`} kind="book" title={book.title} description={book.subtitle || book.description} badge={`${book._count.items} chapters`} meta={formatRelativeTime(book.updatedAt)} favorite><BookOpen className="h-4 w-4 text-accent-dark" /></ResourceCard>)}</div></section>}
          {notes.length > 0 && <section><SectionHeader title="Notes" description={`${notes.length} saved ${notes.length === 1 ? "note" : "notes"}`} /><div className="resource-list">{notes.map((note) => <ResourceCard key={note.id} href={`/notes/${note.id}`} kind="note" title={note.title} description={note.description || note.originalFilename} meta={formatRelativeTime(note.updatedAt)} favorite><TagList tags={note.tags.map(({ tag }) => tag)} /></ResourceCard>)}</div></section>}
          {reviewers.length > 0 && <section><SectionHeader title="Reviewers" /><div className="resource-list">{reviewers.map((reviewer) => <ResourceCard key={reviewer.id} href={`/reviewers/${reviewer.id}`} kind="reviewer" title={reviewer.title} description={reviewer.description} badge={reviewer.style} meta={formatRelativeTime(reviewer.updatedAt)} favorite><TagList tags={reviewer.tags.map(({ tag }) => tag)} /></ResourceCard>)}</div></section>}
          {quizzes.length > 0 && <section><SectionHeader title="Quizzes" /><div className="resource-list">{quizzes.map((quiz) => <ResourceCard key={quiz.id} href={`/quizzes/${quiz.id}`} kind="quiz" title={quiz.title} description={quiz.description} badge={quiz.mode.replace("_", " ")} meta={formatRelativeTime(quiz.updatedAt)} favorite><TagList tags={quiz.tags.map(({ tag }) => tag)} /></ResourceCard>)}</div></section>}
        </div>
      )}
    </PageShell>
  );
