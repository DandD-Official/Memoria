...
> = {
  note: {
    label: "Note",
    icon: Info,
    container: "border-ink/10 bg-ink/[0.03] border-s-ink/30",
    icon_: "text-ink-soft",
  },
  info: {
    label: "Info",
    icon: Info,
    container: "border-accent/30 bg-accent-soft/20 border-s-accent",
    icon_: "text-accent-dark",
  },
  tip: {
    label: "Tip",
    icon: Lightbulb,
    container: "border-accent/50 bg-accent-soft/40 border-s-accent-dark",
    icon_: "text-accent-dark",
  },
  success: {
    label: "Success",
    icon: CheckCircle2,
    container: "border-success/40 bg-success/10 border-s-success",
    icon_: "text-success",
  },
  warning: {
    label: "Warning",
    icon: AlertTriangle,
    container: "border-accent-dark/50 bg-accent-soft/60 border-s-accent-dark",
    icon_: "text-accent-dark",
  },
  danger: {
    label: "Danger",
    icon: AlertOctagon,
    container: "border-danger/50 bg-danger/10 border-s-danger",
    icon_: "text-danger",
  },
};
...
export function CalloutBlock({ node, children }: { node: MmdBlockNode; children: ReactNode }) {
  ...
  return (
    <aside className={cn("my-5 rounded-card border border-s-4 p-5 shadow-sm", config.container, mmdPresentationClass(node.attrs))} aria-label={title || config.label}>
