...
export function BookSurface({ book, assetRegistry }: { book: BookDocument; assetRegistry?: MmdAssetRegistry }) {
  return <div className={`mmd-export-surface book-paper ${book.kind === "NOTEBOOK" ? "notebook-paper" : ""}`} data-book-source>
    <section className={`${book.kind === "NOTEBOOK" ? "notebook-cover " : ""}book-cover${book.title.length + (book.subtitle?.length ?? 0) + (book.description?.length ?? 0) > 900 ? " book-cover-compact" : ""}`} data-book-cover>
      ...
      <div className="book-cover-heading"><p className="book-small-label">{book.kind === "NOTEBOOK" ? "A notebook of connected subjects" : "A collection of connected ideas"}</p><h1>{book.title}</h1>{book.subtitle && <p className="book-cover-subtitle">{book.subtitle}</p>}</div>
    ...
    <section className="book-contents-source" data-book-contents>
      ...
      <ol>{book.chapters.map((chapter, index) => <li key={chapter.id} data-book-toc-row={chapter.id}><span className="book-toc-number">{String(index + 1).padStart(2, "0")}</span><div><span className="book-small-label">{chapter.subjectTitle ? chapter.subjectTitle + " / " : ""}{chapter.kind === "NOTE" ? "Source note" : chapter.kind === "REVIEWER" ? "Study guide" : "Practice"}</span><strong><a href={`#chapter-${chapter.id}`} data-book-goto={chapter.id}>{chapter.title}</a></strong></div><span className="book-toc-page" data-book-page-for={chapter.id}>—</span></li>)}</ol>
    ...
    <CodeThemeProvider>{book.chapters.map((chapter, index) => <section key={chapter.id} data-book-chapter={chapter.id} className="book-chapter-source">
      <div className="mmd-export-flow">
        <header className="mmd-export-brand-header book-chapter-kicker"><span>{chapter.subjectTitle ? `${chapter.subjectTitle} / ` : ""}CHAPTER {String(index + 1).padStart(2, "0")}</span><span>{chapter.kind === "QUIZ" ? "Practice & recall" : "Read & connect"}</span></header>
