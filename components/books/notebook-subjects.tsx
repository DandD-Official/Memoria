...
export function NotebookSubjects({ subjects, save, busy }: { subjects: NotebookSubject[]; save: (subjects: NotebookSubject[]) => Promise<boolean>; busy: boolean }) {
  ...
  return <section className="mb-6 rounded-card border border-line bg-surface-muted p-4" aria-label="Notebook subjects">
    <Label htmlFor="new-subject">Subjects</Label><p className="mb-3 text-sm text-ink-soft">Give each subject a title, then choose the memories that belong inside it.</p>
    <div className="flex gap-2"><Input id="new-subject" placeholder="Subject title, e.g. Biology" value={title} maxLength={120} onChange={event => setTitle(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !busy) void add(); }} /><Button disabled={busy || !title.trim() || subjects.length >= 50} onClick={() => void add()}><Plus className="h-4 w-4" />Add</Button></div>
