...
export function AccountSettings({ initial }: { initial: { name: string; email: string } }) {
  ...
  async function deleteAccount() {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Couldn't reach the server."); setBusy(null); throw caught; }
  }
  ...
  return <section id="account" className="card mt-5 scroll-mt-24 p-5">
    ...
    <div className="mt-8 rounded-card border border-danger/30 p-4"><h3 className="text-sm font-semibold text-danger">Delete account</h3><p className="mt-1 text-xs text-ink-soft">This permanently removes Memories, Books, reviewers, quizzes, attempts, and connections.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><Input type="password" value={deletePassword} onChange={(event) => setDeletePassword(event.target.value)} placeholder="Confirm with your password" /><ConfirmDialog trigger={<Button variant="danger" disabled={!deletePassword} loading={busy === "delete"}>Delete account</Button>} title="Delete your Memoria account?" description="Everything in this account will be permanently removed. This cannot be undone." confirmLabel="Permanently delete" destructive onConfirm={deleteAccount} /></div></div>
  </section>;
}
