  ...
  return (
    <div className="space-y-5">
      ...
      <section id="navigation" className="card scroll-mt-24 p-5">
        ...
        <p className="mb-4 mt-1 text-sm text-ink-soft">Choose how the library index opens and how much space pages use.</p>
        <Label>Library index behavior</Label>
        <div className="grid gap-2 sm:grid-cols-2">
          <button type="button" onClick={() => save({ sidebarMode: "MANUAL" })} className={cn("rounded-lg border p-3 text-left", settings.sidebarMode === "MANUAL" ? "border-action bg-accent-soft" : "border-line hover:bg-ink/5")}><span className="block text-sm font-medium text-ink">Manual button</span><span className="mt-1 block text-xs text-ink-soft">Use the Library index button to show or hide its links.</span></button>
          <button type="button" onClick={() => save({ sidebarMode: "HOVER" })} className={cn("rounded-lg border p-3 text-left", settings.sidebarMode === "HOVER" ? "border-action bg-accent-soft" : "border-line hover:bg-ink/5")}><span className="block text-sm font-medium text-ink">Open on hover</span><span className="mt-1 block text-xs text-ink-soft">Move the pointer over the library index to reveal its links. The button also works with a keyboard.</span></button>
        </div>
        {settings.sidebarMode === "MANUAL" && <label className="mt-3 flex items-start gap-3 rounded-lg border border-line p-3 text-sm text-ink"><input className="mt-0.5 h-4 w-4 accent-accent" type="checkbox" checked={settings.sidebarCollapsed} onChange={(e) => save({ sidebarCollapsed: e.target.checked })} /><span><span className="block font-medium">Start with library index collapsed</span><span className="block text-xs text-ink-soft">Keep the library links tucked away until you open the index.</span></span></label>}
