...
import { measureLayoutTree, cloneLayoutWindow } from "./page-fragments";
...
export function buildGeometricPages(sourceFlow:HTMLElement):GeometricPage[]{
  const stage=document.createElement("div");stage.className=`mmd-export-surface${sourceFlow.closest(".book-paper")?" book-paper":""}${sourceFlow.closest(".notebook-paper")?" notebook-paper":""}`;stage.dataset.renderMode="export";stage.style.cssText="position:fixed;left:-100000px;top:0;width:794px;pointer-events:none";stage.inert=true;
  ...
  try{
    ...
    let geometry=measure(flow);
    const planPages = () => planPageBreaks(geometry.candidates, EXPORT_PAGE.contentHeight, {
      totalHeight: geometry.total, indivisible: geometry.ranges,
      occupied: geometry.ranges.filter(range => range.kind !== "heading"),
      continuationSpace: from => Math.max(0, ...continuationHeaders(geometry, from).map(t => t.headerHeight)),
    });
    let plans=planPages();
    if(plans.some(page=>page.oversized)){
      // Staggered columns can have no shared safe line boundary. Stack only in
      // that case, retaining each column's frame and spacing at readable size.
      for(const columns of flow.querySelectorAll<HTMLElement>('[data-export-block="columns"]'))columns.style.gridTemplateColumns="minmax(0,1fr)";
      fitExportTables(flow);geometry=measure(flow);plans=planPages();
    }
    if(plans.some(page=>page.oversized)){
      // A heading + an almost-page-height row cannot both fit. Keep the row
      // and its text intact, relaxing only the heading preference.
      geometry.ranges=geometry.ranges.filter(range=>range.kind!=="heading");plans=planPages();
    }
    const tree=measureLayoutTree(flow);
    return plans.map(plan=>{
      ...
      const copy=cloneLayoutWindow(tree,plan.from,plan.to,geometry.top);copy.style.width=`${EXPORT_PAGE.cssWidth}px`;copy.style.position="relative";copy.style.left=`-${EXPORT_PAGE.margin}px`;copy.style.transform=`translateY(-${plan.from+EXPORT_PAGE.margin}px)`;copy.style.transformOrigin="top left";viewport.append(copy);content.append(viewport);
