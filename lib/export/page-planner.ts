...
export interface PagePlanOptions { totalHeight?:number; indivisible?:IndivisibleRange[]; continuationSpace?:(start:number)=>number; minFill?:number }
...
export function planPageBreaks(candidates:BreakCandidate[],pageHeight:number,options:PagePlanOptions={}):PlannedPage[]{
  ...
  while(from<total-.05){
    ...
    if(to===undefined||(to-from+reserve)/pageHeight<(options.minFill??.75))to=forced?.y??available.at(-1)?.y??to;
